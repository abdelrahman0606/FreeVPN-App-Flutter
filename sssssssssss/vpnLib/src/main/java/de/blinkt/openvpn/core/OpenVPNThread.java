...
public class OpenVPNThread implements Runnable {
    ...
    private void startOpenVPNThreadArgs(String[] argv) {
        ...
        try {
            Log.d(TAG, "Starting OpenVPN with command: " + String.join(" ", argvlist));
            mProcess = pb.start();
            Log.d(TAG, "OpenVPN process started, PID: " + (mProcess != null ? "valid" : "null"));
            ...
            while (true) {
                ...
                if (logline == null) {
                    // Process output stream closed - check if process is still alive
                    if (mProcess != null) {
                        try {
                            int exitValue = mProcess.exitValue();
                            Log.e(TAG, "OpenVPN process exited unexpectedly with exit code: " + exitValue);
                            VpnStatus.logError("OpenVPN process exited with code: " + exitValue);
                        } catch (IllegalThreadStateException e) {
                            // Process is still running, stream just closed
                            Log.w(TAG, "OpenVPN output stream closed but process still running");
                        }
                    }
                    return;
                }
